...
function ResultadoBusquedaReferencia(props){
    ...
    React.useEffect(() => {
        console.log(props.expresionesEncontradas)
        console.log(props.idPasaje)
    }, [props.posicionPasaje])
    

    function htmlPasajeOriginal(){
        return {__html:props.expresionesEncontradas.ref_original}
    }

    function htmlPasajeTraduccion(){
        return {__html:props.expresionesEncontradas.ref_traduccion}
    }

    return(
        <Grid container>
             <Grid item xs={12} className="pasajesRenderizadosBusqueda">
                <Typography variant="h4" className={classes.typosTitulos}>{props.expresionesEncontradas.ref_id}</Typography>
            ...
            <Grid item xs={12} className="pasajesRenderizadosBusquedaPorReferencia">
                ...
                <ul className="ulExpresionesRelacionadas">
                    {props.expresionesEncontradas.expresiones.map(expresion=>(
                        <li
                            key={expresion.t_id}
                            className="liExpresionesRelacionadas"
                        >
                            <Link to={`/husserl/pasaje/${expresion.t_id}/${props.expresionesEncontradas.ref_id}`}>
